...
int main(void) {
	...
	pbin8_nb (x, "23 - take an integer");
	pbin8_nb (-x, "-23 - when minussed");
	pbin8_nb (~x, "~23 then inversed");
	pbin8_nb (~x+1, "~23 and add +1");
	printf( (-x == ~x+1) ? "a) -x and ~x+1 are the same\n": "-x == ~x+1 NOT the same\n");

	pbin8_nb (~(x-1), "~23 or inverse (that number - 1)");
	printf( (-x == ~(x-1)) ? "b) -x and ~(x-1) are the same\n": "-x == ~(x-1) NOT the same\n");

	
	pbin8_nb (~x, "~23 inverse a number");
	pbin8_nb (~x, "then minus it and 1");
	...
	printf("\n1. x+y are equal to :\n");
	pbin8_nb(x+y, "x + y (23+13) : ");
	pbin8_nb(x-~y-1, "f) x-~y-1");
	pbin8_nb((x^y) + 2*(x&y), "g) (x^y) + 2*(x&y)");
	pbin8_nb((x|y) + (x&y), "h) (x|y) + (x&y)") ;
	pbin8_nb(2*(x|y) - (x^y), "i) 2*(x|y) - (x^y)") ;
	
	printf("\n2. x-y are equal to :\n");
	pbin8_nb(x-y, "x - y (23-13) : ");
	pbin8_nb(x+~y+1, "j) x+~y+1");
	pbin8_nb((x^y) - 2*(~x&y), "k) (x^y) - 2*(~x&y)");
	pbin8_nb((x&-y) - (-x&y), "l) (x&-y) - (-x&y)") ;
	pbin8_nb(2*(x&~y) - (x^y), "m) 2*(x|y) - (x^y)") ;
